...
while True:
    ...
    if ((d.checkVertical(x, test)) and
        ...
        # This finds an possible number that hasn't been checked
        # already. Possible in possibilities, already checked in
        # alreadyChecked; one that's in possible but not in checked.
    ...
    # If there already exists the current number in the puzzle at row, col, &/or quad.,
    # log the guess into AlreadyChecked for that point and
    # increment the iterator by one to check another number out of the possibilities.
    else:
        alreadyChecked[y][x][i] = test
        i += 1
...
# These print out the entire rows of the puzzle.
print(sudPuzzle[0][:])
print(sudPuzzle[1][:])
print(sudPuzzle[2][:])
print(sudPuzzle[3][:])
