...
puzzle = np.array([[0,0,0,3,0,0,0,4,0],
          [0,0,3,2,0,0,0,0,0],
          [7,5,0,0,8,1,0,0,0],
          [6,8,0,9,0,3,1,5,0],
          [0,0,5,0,0,0,9,0,0],
...
# +These tell me which numbers are missing from each
# quadrant in the puzzle
c = Check(puzzle)
...
# missing in the quadrants, which I then plug in to the place of the zeros
...
# + I have split the for loops into two separate batches
# in order to dramatically increase the speed at which this program runs.

# + These two for loops check quadrants 1-3 & 4-6, and saves the ones
# that work for each other the initial givens.
...
# These two for loops take the solutions from the top two rows of quadrants
# and checks them with the bottom row, 7-9, to see which ones work together.
# + There will only be one arrangement in the puzzle that has all 81 numbers
# appear once in the row, column, and quadrant, and that is what will come out of
...
# + Print the solutions that you just found in the program.
print('SOLUTION:')
print(answer1)
print(answer2)
print(answer3)
...
# puzzle that my program came up with, in the more easily readable
# form that the sudoku puzzle is usually given in.
...
# This gives the total time that the program took to run from start
# to finish.
end = time.time()
